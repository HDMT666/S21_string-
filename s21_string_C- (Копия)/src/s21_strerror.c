...
#define UNKNOWN_ERROR_INDEX MESSAGES_COUNT
...
char* s21_strerror(int errnum) {
  char* message[MESSAGES_COUNT + 1] = ERROR_MESSAGES;

  if (errnum < 0 || errnum > MESSAGES_COUNT - 1) {
    char* buffer = (char*)malloc(BUFFER_SIZE * sizeof(char));
    s21_strncpy(buffer, UNKNOWN_ERROR, BUFFER_SIZE);

    char numberBuffer[OFFSET];
    to_string(errnum, numberBuffer);
    s21_strncat(buffer, numberBuffer, BUFFER_SIZE - s21_strlen(buffer) - 1);

    message[UNKNOWN_ERROR_INDEX] = buffer;
    errnum = UNKNOWN_ERROR_INDEX;
  }
